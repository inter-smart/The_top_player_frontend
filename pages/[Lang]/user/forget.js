...
import { useDispatch } from "react-redux";
import { ClearSecret, LogOutReducer, VerifyEmail } from "@/store/AuthSlice";
      ...
      >
        ...
        <div className={styles.Login_card}>
          ...
          <form onSubmit={formik.handleSubmit} className="grid  gap-2">
            ...
            <button name="login" type="submit" className={`submit-button ${disabel == true && "LoadingButton"}`}>
              {t("auth.send")}
            </button>
            {/* <div className={styles.have_account}>
              ...
              >
                {t("auth.have_account")}
                <Link href={`/${Lang}/auth/login`}>{t("auth.login")}</Link>
              </p>
            </div> */}
          </form>
        </div>

...
export async function getServerSideProps({ req, params }) {
  const { programs_id, sub, Lang } = params;
  const parsedProgramId = parseInt(programs_id);
  const parsedSubId = parseInt(sub);
  ...
  try {
    const [courseRes, subCourseRes, courseByIdRes] = await Promise.all([
      axios.get(`${process.env.customKey}/course/${parsedProgramId}`, axiosConfig),
      axios.get(`${process.env.customKey}/subcourse/${parsedProgramId}/${parsedSubId}`, axiosConfig),
      axios.get(`${process.env.customKey}/courseById/${parsedProgramId}`, axiosConfig).catch(() => ({ data: { course: null } })),
    ]);

    const courseData = courseRes.data;
    const subCourseData = subCourseRes.data;
    const courseByIdData = courseByIdRes.data.course;

    return {
      props: {
        CoursecArr: courseData,
        SubCourseArr: subCourseData,
        CourseByIdArray: courseByIdData,
        isPurchased: courseData?.isPurchased || null,
        programs_id,

...
const Profile = ({ lang }) => {
  ...
  const formik = useFormik({
    initialValues: { F_name: "", phone: "", message: "", email: "" },
    validate: (data) => {
      let errors = {};
      if (!data.F_name) errors.F_name = t("auth.req_name");
      if (!data.phone) errors.phone = t("auth.req_mobile");
      return errors;
    },
    onSubmit: (data) => {
      setIsSubmitting(true);
      const sanitizedData = {
        username: DOMPurify.sanitize(data.F_name),
        mobile: DOMPurify.sanitize(data.phone),
        bio: DOMPurify.sanitize(data.message),
      };
      dispatch(updateUserInfo(sanitizedData))
        .unwrap()
        .then(() => {
          toast.current.show({ severity: "success", summary: t("auth.succ_update") });
          formik.resetForm();
        })
        .catch((err) => {
          const message = err?.response?.data?.message || t("auth.wrong");
  ...
  const getFormErrorMessage = (name) =>
    isFormFieldInvalid(name) ? <small className="p-error">{formik.errors[name]}</small> : "";
  ...
  useEffect(() => {
    if (!formik.values.F_name) {
      dispatch(getUserInfo())
        .unwrap()
        .then((res) => {
          formik.setFieldValue("F_name", res?.username || "");
          formik.setFieldValue("phone", res?.mobile || "");
          formik.setFieldValue("message", res?.bio || "");
          formik.setFieldValue("email", res?.email || "");
        })
        .catch((err) => {
          console.error("Failed to fetch user info:", err.message); // Improved logging
      ...
      <div className="inner_section_outer">
        <div className={styles.profile_section}>
          <div className="container">
            <Toast ref={toast} />
            <div className={styles.profile_box} style={{ direction: lang === "ar" ? "rtl" : "ltr" }}>
                ...
                <div className={`${styles.formRow} row`}>
                  <div className="col-md-4">
                    <div className="inputFormik">
                      ...
                      <InputText
                        name="F_name"
                        className={classNames({ "p-invalid": isFormFieldInvalid("F_name") })}
                        value={formik.values.F_name}
                        onChange={(e) => formik.setFieldValue("F_name", e.target.value)}
                      />
                  ...
                  <div className="col-md-4">
                    <div className="inputFormik">
                      ...
                      <InputText name="email" value={formik.values.email} disabled />
                    </div>
                  </div>
                  <div className="col-md-4">
                    <div className="inputFormik">
                  ...
                  <div className="col-12">
                    <div className="inputFormik">
                      <label htmlFor="message">{t("auth.about")}</label>
                      <InputTextarea
                        name="message"
                  ...
                  <div className="col-12">
                    ...
                    >
                      {isSubmitting ? t("auth.submitting") : t("auth.send")}
                    </button>
                  </div>
                </div>
              </form>

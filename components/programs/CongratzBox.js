...
import { getCongratsData } from "@/store/CongratsSlice";

const CongratsBox = ({ type, Lang }) => {
        ...
        <div className={styles.enrollProgramBx}>
          ...
          <div className={styles.lftWrap}>
            <div className={`${styles.tleWrap} tleWrap`}>
              ...
              {congrats ? (
                <div className={`${styles.mTle} mTle `}>{Lang == "ar" ? congrats?.heading_ar : congrats?.heading}</div>
              ) : (
                <div className={`${styles.mTle} mTle `}>{t("programs.congratulations")}</div>
              )}
            </div>
            <div className={styles.imgBx}>
              {congrats ? (
                <div className={styles.dElmt}>
                  <Image className={styles.elmt_1} src={`${process.env.customKey}/congrats_images/${congrats?.imageUrl}`} width={756} height={448} alt="Congratulations" />
                </div>
              ) : (
                <div className={styles.dElmt}>
                  <Image className={styles.elmt_1} src="/images/congratulation-Image.jpg" width={756} height={448} alt="Congratulations" />
                </div>
              )}
            </div>
            {congrats ? (
              <div className={styles.txt}>{Lang == "ar" ? congrats?.sub_text_ar : congrats?.sub_text}</div>
            ) : (

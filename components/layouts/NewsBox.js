...
const NewsBox = ({ Lang = "en", news = {} }) => {
  const { t } = useTranslation();
  const safeLang = typeof Lang === "string" && Lang.length <= 5 ? Lang.toLowerCase() : "en";
  const safeNews = news && typeof news === "object"
    ? {
        title_en: sanitizeString(news.title_en || ""),
        title_ar: sanitizeString(news.title_ar || ""),
        description_en: sanitizeString(news.description_en || ""),
        description_ar: sanitizeString(news.description_ar || ""),
        coverimage: news.coverimage && typeof news.coverimage === "string"
          ? sanitizeString(news.coverimage)
          : "/images/default-image.jpg",
        createdAt: news.createdAt && !isNaN(new Date(news.createdAt).getTime())
    ...
    : {
        ...
        description_en: "",
        description_ar: "",
        coverimage: "/images/default-image.jpg",
        createdAt: new Date(),
      };
  ...
  return (
    <div className={styles.newsBx}>
      ...
      <div className={styles.imgWrap}>
        <Image
          src={`${process.env.customKey}/newsCoverImages/${safeNews.coverimage}`}
          alt={safeLang === "ar" ? safeNews.title_ar : safeNews.title_en}
          layout="fill"
          objectFit="cover"
          loading="lazy"
        />
      ...
      <div className={styles.cntWrap}>
        <div className={styles.tle}>
          {safeLang === "ar" ? safeNews.title_ar : safeNews.title_en}
        </div>
        <div className={styles.txt}>
          {safeLang === "ar" ? safeNews.description_ar : safeNews.description_en}
        </div>
        <div className={styles.info}>
          {t("news.postdate")} : {format(safeNews.createdAt, "dd MMMM yyyy")}
        </div>
        <Link
          href="#news"
        ...
        >
          ...
          <span className="icon">
            <svg
              ...
              style={{
                transform: safeLang === "ar" ? "rotate(180deg)" : "rotate(0)",
              }}
            >
